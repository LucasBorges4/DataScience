...
def process_image_ndvi(im, area_km2):
    # Converte a imagem para numpy array e para tipo float
    ...
    # Filtro gaussiano
    ndvi = cv2.GaussianBlur(ndvi, (5,5), 0.8)

    # K-Means
    K = 3

    # Reorganiza para passar ao k-means
    ndvi_reshaped = ndvi.reshape((rows * cols, 1))
    kmeans = KMeans(n_clusters=K, random_state=0).fit(ndvi_reshaped)
    labeled = kmeans.labels_.reshape((rows, cols))
